    ...
    <div>
        <el-form ref="user" :model="user" :rules="rules" label-width="80px" size="mini">
            <el-form-item label="atid" prop="atid">
                <el-input v-model="user.atid" readonly></el-input>
            </el-form-item>
            <el-form-item label="emid" prop="emid">
                <el-input v-model="user.emid" readonly></el-input>
            </el-form-item>
...
<script>
    export default {
        ...
        methods: {
            onSubmit(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.post('http://localhost:8181/Attendance/update',this.user).then(function (resp) {
                            console.log(_this.user.emid)
                            console.log(resp)
                            if(resp.data.emid==_this.user.emid){
                                _this.$message({
                                    message: _this.user.atid+'修改成功',
                                    type: 'success'
                                });
        ...
        created() {
            ...
            alert( this.$route.query.atid);
            const _this=this;
            axios.get('http://localhost:8181/Attendance/update/'+this.$route.query.atid).then(function (resp) {
