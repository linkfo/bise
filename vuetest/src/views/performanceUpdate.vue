...
<script>
    export default {
        ...
        methods: {
            onSubmit(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.post('http://localhost:8181/Performance/update',this.user).then(function (resp) {
                            console.log(_this.user.peid)
                            console.log(resp)
                            if(resp.data.peid==_this.user.peid){
                                _this.$message({
                                    message: _this.user.peid+'修改成功',
                                    type: 'success'
                                });
        ...
        created() {
            alert( this.$route.query.peid);
            const _this=this;
            axios.get('http://localhost:8181/Performance/update/'+this.$route.query.peid).then(function (resp) {
