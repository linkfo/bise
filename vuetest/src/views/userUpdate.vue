...
<script>
    export default {
        ...
        methods: {
            onSubmit(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.post('http://localhost:8181/Employee/update',this.user).then(function (resp) {
                            console.log(_this.user.emid)
                            console.log(resp)
                            if(resp.data.emid==_this.user.emid){
                                _this.$message({
                                    message: _this.user.emid+'修改成功',
                                    type: 'success'
                                });
        ...
        created() {
            ...
            alert( this.$route.query.emid);
            const _this=this;
            axios.get('http://localhost:8181/Employee/update/'+this.$route.query.emid).then(function (resp) {
